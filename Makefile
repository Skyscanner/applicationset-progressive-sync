...
shftm: shfmt ## Run shfmt against code.
	$(SHFMT) -l -d hack/.
...
test: manifests generate fmt shfmt vet ginkgo ## Run tests.
	...
	source ${ENVTEST_ASSETS_DIR}/setup-envtest.sh; fetch_envtest_tools $(ENVTEST_ASSETS_DIR); setup_envtest_env $(ENVTEST_ASSETS_DIR); ${GINKGO} -v -r --failOnPending --cover -coverprofile=../coverage.out --trace --race
...
shfmt: ## Download shfmt locally if necessary.
	$(call go-install-tool,$(SHFMT),mvdan.cc/sh/v3/cmd/shfmt@v3.4.0)
